...
use crate::mods::{scan_disabled_mods, scan_mods, ModInfo};
...
// `crate::profiles::write_profile_load_order_to_settings_file`, etc.
// surface that the rest of the codebase reaches via the same paths it
// used pre-split.
...
pub(super) fn disk_mod_matches_pin(
...
pub(super) async fn switch_profile_from_paths(
    name: &str,
    mods_path: &Path,
    disabled_path: &Path,
    profiles_path: &Path,
...
mod modpack_flow_tests {
    use super::*;
    use super::drift::{compute_profile_drift, repair_profile_from_paths};
