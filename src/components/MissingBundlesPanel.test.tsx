...
describe('<MissingBundlesPanel>', () => {
  const baseProfile: Profile = {
    ...
    public: null,
    mods: ['A', 'B', 'C', 'GoodMod', 'BadMod', 'BrokenMod', 'StuckMod', 'ModA', 'ModB'].map(
      (name) => ({
        name,
        version: '1.0',
  ...
  it('upload failures keep "Try sharing again" as the recovery action', async () => {
    ...
    expect(
      getInvokeCalls().filter((c) => c.cmd === 'restore_profile_mod_from_share'),
    ).toHaveLength(0);
  });

  it('local missing files offer remove and reinstall actions', () => {
    render(<Wrap modNames={['A']} />);
    const removeBtn = screen.getByRole('button', { name: /Remove from modpack/i });
    const reinstallBtn = screen.getByRole('button', { name: /Reinstall missing mods/i });
  ...
  it('"Reinstall missing mods" restores every mod from the shared pack/source', async () => {
    ...
    await waitFor(() => {
      const calls = getInvokeCalls().filter((c) => c.cmd === 'restore_profile_mod_from_share');
      expect(calls.map((c) => c.args)).toEqual([
        { profileId: 'profile-1', modName: 'A', folderName: 'AFolder', modId: 'A-id' },
        { profileId: 'profile-1', modName: 'B', folderName: 'BFolder', modId: 'B-id' },
        { profileId: 'profile-1', modName: 'C', folderName: 'CFolder', modId: 'C-id' },
      ]);
    });
  });
  ...
  it('"Remove from modpack" removes every stale row and auto-retries the publish', async () => {
    ...
    const user = userEvent.setup();
    render(<Wrap modNames={['A', 'B']} onRetryPublish={onRetryPublish} />);
    await user.click(screen.getByRole('button', { name: /Remove from modpack/i }));
    await waitFor(() => {
  ...
  it('partial remove failure keeps the panel open without auto-retry', async () => {
    ...
    const onRetryPublish = vi.fn().mockResolvedValue(undefined);
    const user = userEvent.setup();
    render(<Wrap modNames={['GoodMod', 'BadMod']} onRetryPublish={onRetryPublish} />);
    await user.click(screen.getByRole('button', { name: /Remove from modpack/i }));
    await waitFor(() => {
      expect(screen.getByText(/Failed/i)).toBeInTheDocument();
    });
