...
/// Repair result for a single mod folder.
...
/// Scan mods for folder name mismatches and rename them to match
/// their DLL/PCK stem (what the game actually loads).
/// This fixes folders created with Nexus download filenames like
/// "STS2-Ritsu-281-0-0-46-1775500710" that should be "STS2-RitsuLib".
#[tauri::command]
pub fn repair_mod_folders(state: tauri::State<'_, AppState>) -> std::result::Result<Vec<RepairResult>, String> {
    let s = state.lock().map_err(|e| e.to_string())?;
    let mods_path = s.mods_path.as_ref().ok_or("Game path not set")?.clone();
    let disabled_path = s.disabled_mods_path.as_ref().ok_or("Game path not set")?.clone();
    drop(s);
    ...
    for search_path in [&mods_path, &disabled_path] {
        ...
        let entries: Vec<_> = match fs::read_dir(search_path) {
        ...
        for entry in entries {
            ...
            // Look for a DLL inside this directory — its stem is the canonical name
            let mut dll_stem: Option<String> = None;
            ...
            if let Ok(sub_entries) = fs::read_dir(&path) {
                for sub_entry in sub_entries.flatten() {
                    ...
                    let ext = sub_path.extension()
                        .and_then(|e| e.to_str())
                        .unwrap_or("")
                        .to_lowercase();

                    if (ext == "dll" || ext == "pck") && dll_stem.is_none() {
                        if let Some(stem) = sub_path.file_stem() {
                            dll_stem = Some(stem.to_string_lossy().to_string());
                        }
                    }

                    if ext == "json" && manifest_id.is_none() {
                        if let Ok(content) = fs::read_to_string(&sub_path) {
                            if let Ok(val) = serde_json::from_str::<serde_json::Value>(&content) {
                                manifest_id = val.get("Id")
                                    .or_else(|| val.get("id"))
                                    .and_then(|v| v.as_str())
                                    .filter(|s| !s.is_empty())
                                    .map(|s| s.to_string());
            ...
            // The correct folder name should match the DLL stem
            let correct_name = dll_stem.or(manifest_id);

            if let Some(ref correct) = correct_name {
                if correct != &dir_name {
                    let new_path = search_path.join(correct);

                    // Don't rename if destination already exists
                    if new_path.exists() {
                        log::warn!(
                            "Repair: wanted to rename '{}' -> '{}' but destination exists, removing duplicate",
                            dir_name, correct
                        );
                        // The correct folder already exists — remove the misnamed duplicate
                        let _ = fs::remove_dir_all(&path);
                        repairs.push(RepairResult {
                            old_folder: dir_name.clone(),
                            new_folder: format!("{} (removed duplicate)", correct),
                            mod_name: correct.clone(),
                        });
                        continue;
                    }
                    ...
                    match fs::rename(&path, &new_path) {
                        Ok(()) => {
                            log::info!("Repaired mod folder: '{}' -> '{}'", dir_name, correct);
                            repairs.push(RepairResult {
                                old_folder: dir_name,
                                new_folder: correct.clone(),
                                mod_name: correct.clone(),
                            });
                        }
                        Err(e) => {
